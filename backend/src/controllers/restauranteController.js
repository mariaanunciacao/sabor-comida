import { Restaurante, RestauranteEndereco, Usuario, sequelize } from '../models/index.js';
...
function isCompleteAddress(payload) {
    return ['logradouro', 'numero', 'cep', 'cidade', 'estado'].every((field) => normalizeText(payload?.[field]).length > 0);
}
...
function buildRestaurantResponse(restaurante, usuario) {
    ...
    const statusAprovacao = plainRestaurant.statusAprovacao ?? 'pendente';
    ...
    if (statusAprovacao === 'aprovado') {
        ...
        message = 'Seu cadastro foi rejeitado. Você pode revisar os dados e reenviar.';
    }
...
export async function statusMeuRestaurante(req, res) {
    const tipoUsuario = String(req.auth?.tipo ?? req.auth?.perfil ?? '').trim();

    if (!ALLOWED_USER_TYPES.includes(tipoUsuario)) {
        return res.status(403).json({ message: 'Somente usuários de restaurante podem consultar este status.' });
    }
...
export async function meuRestaurante(req, res) {
    ...
    if (!ALLOWED_USER_TYPES.includes(tipoUsuario)) {
        return res.status(403).json({ message: 'Somente usuários de restaurante podem acessar este recurso.' });
    }
...
export async function atualizarMeuRestaurante(req, res) {
    ...
    if (!ALLOWED_USER_TYPES.includes(tipoUsuario)) {
        return res.status(403).json({ message: 'Somente usuários de restaurante podem enviar dados para análise.' });
    }
    ...
    const nomeRestaurante = normalizeText(req.body?.nome_restaurante);
    ...
    const horarioAtendimento = normalizeText(req.body?.horario_funcionamento ?? req.body?.horario_atendimento);
    ...
    if (!nomeRestaurante || !cnpj || !horarioAtendimento || !tempoEntrega || !isCompleteAddress(enderecoPayload)) {
        return res.status(400).json({ message: 'Informe nome do restaurante, CNPJ, horário, tempo de entrega e endereço completo.' });
    }
    ...
    const restaurante = await sequelize.transaction(async (transaction) => {
        ...
        if (!targetRestaurant) {
            ...
            targetRestaurant.nome_restaurante = nomeRestaurante;
            targetRestaurant.cnpj = cnpj;
            targetRestaurant.descricao = descricao || null;
            targetRestaurant.logo_path = logoPath || null;
            targetRestaurant.banner_path = bannerPath || null;
            targetRestaurant.horario_atendimento = horarioAtendimento;
            targetRestaurant.tempo_entrega = tempoEntrega;
            targetRestaurant.status_aprovacao = 'pendente';
            await targetRestaurant.save({ transaction });
        }

        const [endereco] = await RestauranteEndereco.findOrCreate({
            where: { idRestaurante: targetRestaurant.id },
            defaults: {
        ...
        endereco.logradouro = enderecoPayload.logradouro;
        endereco.numero = enderecoPayload.numero;
        endereco.cep = enderecoPayload.cep;
        endereco.cidade = enderecoPayload.cidade;
        endereco.estado = enderecoPayload.estado;
        await endereco.save({ transaction });

        await usuario.update(
            {
                tipo: usuario.tipo === 'admin' ? 'admin' : 'restaurante_pendente',
            },
            { transaction },
        );
    ...
    const updatedRestaurant = await Restaurante.findByPk(restaurante.id, {
        include: restaurantInclude,
    });

    const responsePayload = buildRestaurantResponse(updatedRestaurant, usuario);
